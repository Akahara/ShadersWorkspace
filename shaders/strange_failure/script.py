...
def findFaces(points, maxDist):
	...
	for (i, p1) in enumerate(points):
		for (j, p2) in enumerate(points[i+1:]):
			if np.linalg.norm(p1-p2) < maxDist:
				for p3 in points[j+1:]:
					if np.linalg.norm(p2-p3) < maxDist and np.linalg.norm(p1-p3) < maxDist:
...
def splitIcosahedronFaces(faces):
	...
	for (p1, p2, p3) in faces:
		p4 = (p1+p2)/2; p4 = p4/np.linalg.norm(p4)
		p5 = (p2+p3)/2; p5 = p5/np.linalg.norm(p5)
		p6 = (p3+p1)/2; p6 = p6/np.linalg.norm(p6)
